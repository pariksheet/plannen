...
const VALID_STORAGE = ['local-fs', 'supabase', 's3'];
...
export async function invokeProfileCreate(rawArgs, ctx = {}) {
  ...
  if (storage != null && !VALID_STORAGE.includes(storage)) {
    throw new Error(`profile create: --storage must be one of: ${VALID_STORAGE.join(', ')}`);
  }
  ...
  const storageDefaults = storageBackendDefaultsForMode(mode, storage);
