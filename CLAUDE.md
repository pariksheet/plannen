...
- **Tier 0 daily workflow has three processes.** `bash scripts/pg-start.sh` (embedded Postgres on 54322) → `bash scripts/backend-start.sh` (Plannen Hono backend on 54323) → `npm run dev` (web app on 4321 with Vite proxying `/api`, `/storage/v1`, `/functions/v1` to the backend). All three must be running for the web app to work end-to-end. Stop in reverse order.
