// Scrape a URL for event metadata. Three layers:
//   1. JSON-LD structured data (Schema.org Event) — authoritative when present.
//   2. AI extraction over plain-text content + date hints.
//   3. Regex fallback if AI is unavailable / not configured.
//
// If `event_id` is supplied AND the user has settings configured, we also
// persist the extracted fields onto the existing row.
...
export async function handle(req: Request, ctx: HandlerCtx): Promise<Response> {
  ...
  try {
    ...
    const extractedData = await extractWithLLM(ctx, html, url, dateHints, structured)
    ...
    if (extractedData) {
      ...
      if (event_id) {
        ...
        if (extractedData.title) push('title', extractedData.title)
        if (finalStartDate) push('start_date', finalStartDate)
        if (finalEndDate) push('end_date', finalEndDate)
        const imageUrl = extractedData.image_url || extractedImageUrl
        if (imageUrl) push('image_url', imageUrl)
        if (extractedData.location) push('location', extractedData.location)
        if (extractedData.enrollment_deadline) {
          try {
            const deadline = new Date(extractedData.enrollment_deadline)
            if (!isNaN(deadline.getTime())) push('enrollment_deadline', deadline.toISOString())
          } catch (_e) { /* ignore */ }
        }
        if (cols.length > 0) {
          vals.push(event_id)
          await ctx.db.query(
            `UPDATE plannen.events SET ${cols.join(', ')} WHERE id = $${vals.length}`,
            vals,
          )
        }
      }
