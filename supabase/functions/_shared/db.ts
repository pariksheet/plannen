...
// Supabase auto-injects SUPABASE_DB_URL on cloud; the orchestrator may also set
// DATABASE_URL explicitly. Accept either so the function works regardless.
const pool = new Pool({ connectionString: Deno.env.get('DATABASE_URL') ?? Deno.env.get('SUPABASE_DB_URL') ?? '' })
