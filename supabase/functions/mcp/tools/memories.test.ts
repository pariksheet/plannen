...
describe('memories module', () => {
  it('registers exactly 3 tool definitions', () => {
    expect(memoriesModule.definitions).toHaveLength(3)
  })
  it('names', () => {
    const names = memoriesModule.definitions.map((d) => d.name).sort()
    expect(names).toEqual(['add_event_memory', 'list_event_memories', 'list_event_notes'])
  })
  it('does NOT include transcribe_memory (intentionally local-only — requires whisper.cpp on PATH)', () => {
