...
description: Use when the user asks to write a story, make a story, tell the story of a past event or trip, or compose a narrative about an event, date range, or holiday. Drives the Plannen MCP tools get_event/list_events → list_event_memories + list_event_notes → create_story. Trigger only on explicit request — never auto-generate stories.
---

# Plannen — stories

When the user asks to "write a story", "make a story", or "tell me about" a past event (or a date range / trip), drive the Plannen MCP tools `get_event` (or `list_events` for ranges) → `list_event_memories` + `list_event_notes` → `transcribe_memory` (for audio, best-effort) → `get_story_languages` → `create_story` (one call per language, sharing `story_group_id`). **Only on explicit request** — never auto-generate.
...
2. **Load memories and notes.** For each target event call BOTH `list_event_memories({ event_id })` and `list_event_notes({ event_id })`. Combine across events when multi-event. An empty result on either is fine — generate from what you have. Notes are first-person observations from anyone who can see the event (multi-user on shared events); each row carries `author_full_name` / `author_email` and a `created_at` timestamp. Treat notes as primary narrative material when present — they often contain quotes, reactions, or details that don't appear elsewhere.
