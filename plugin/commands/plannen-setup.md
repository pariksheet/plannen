---
description: Re-configure Plannen — edit your email, Supabase URL/keys, or Google OAuth in .env. Use `npx plannen init` for first-time install.
argument-hint: ""
---

The user has invoked `/plannen-setup`. This command **edits `.env` only** — re-config for an already-installed Plannen. First-time setup goes through `npx plannen init --mode=<m>` instead; if there is no `.env` yet, point the user there.
...
1. **Read `.env`.** If missing, tell the user: *"No `.env` found. This is a first-time install — run `npx plannen init --mode=<local_pg|local_sb|cloud_sb>` from the repo root, not this command."* Stop.
...
- Run a full install. If the user wants that, point them at `npx plannen init --mode=<m>`.
