...
From the repo root, run the one-shot init first — it does the npm install, supabase start, migrations, auth user, env files, functions-serve, and offers to install the plugin at the end:

```
npx plannen init --mode=local_pg     # or local_sb / cloud_sb
```
