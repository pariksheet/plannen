...
cd plannen
npx plannen init --mode=local_pg     # or --mode=local_sb / --mode=cloud_sb
```

`plannen init` is idempotent — re-run it any time things drift. If something looks off later, `/plannen-doctor` (inside Claude Code) prints a targeted diagnosis.

Once it's running:

- Web app: `npm run dev` → http://localhost:4321
- Whole stack (tier-aware lifecycle): `npx plannen up` / `npx plannen down`.
- Backend (Tier 0 only, if you need to bounce it alone): `bash scripts/backend-start.sh` (idempotent)
- Edge functions (Tier 1 only, similarly): `bash scripts/functions-start.sh` (background)
