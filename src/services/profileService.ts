...
export async function upsertProfile(
  updates: { full_name?: string; dob?: string | null; goals?: string[]; interests?: string[]; timezone?: string }
): Promise<{ error: Error | null }> {
  try {
    await dbClient.profile.update(updates as Parameters<typeof dbClient.profile.update>[0])
    return { error: null }
  } catch (e) {
