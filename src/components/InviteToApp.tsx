...
export function InviteToApp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loginUrl = publicUrl ? `${publicUrl}/login` : null
    ...
    if (loginUrl) {
      ...
      setWhatsAppUrl(`https://wa.me/?text=${encodeURIComponent(shareText)}`)
    } else {
      setWhatsAppUrl(null)
    }
    ...
    if (emailErr) {
      setMessage(
        loginUrl
          ...
          : `Invite saved, but we couldn't send the email and no public app URL is configured. Set VITE_APP_URL to enable a shareable invite link.`
      )
    } else {
      setMessage(`Invite email sent to ${trimmed}.${loginUrl ? ' You can also share the link on WhatsApp.' : ''}`)
    }
    setEmail('')
    setName('')
  }
