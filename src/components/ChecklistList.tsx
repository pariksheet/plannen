...
interface Props {
  ...
  /** Optional map of event id → title, to show an "attached event" chip. */
  eventTitleById?: Record<string, string>
}

export function ChecklistList({ checklists, onOpen, onDelete, eventTitleById }: Props) {
        ...
        return (
          <div key={cl.id} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              {cl.event_id && eventTitleById?.[cl.event_id] && (
                <span className="text-xs text-violet-700 bg-violet-50 border border-violet-100 rounded-full px-2 py-0.5 whitespace-nowrap max-w-[40%] truncate" title={eventTitleById[cl.event_id]}>
                  {eventTitleById[cl.event_id]}
                </span>
              )}
