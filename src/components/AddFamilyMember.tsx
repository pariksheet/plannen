...
export function AddFamilyMember({ onSuccess }: AddFamilyMemberProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) return
    setLoading(true)
    const { error: err } = await sendRelationshipRequest(trimmed, 'family')
    setLoading(false)
    if (err) {
      setError(err.message)
      return
    }
    setMessage('Request sent. They’ll see it once they’re in My Family (or when they accept).')
    setEmail('')
    onSuccess()
  }
