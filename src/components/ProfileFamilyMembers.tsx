...
export function ProfileFamilyMembers() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editingId) {
        const id = editingId
        const { error } = await updateFamilyMember(id, payload)
        if (error) { setError(error.message); return }
        setMembers((prev) => prev.map((m) => (m.id === id ? { ...m, ...payload } : m)))
      } else {
        const { data, error } = await addFamilyMember(payload)
        if (error) { setError(error.message); return }
                    ...
                    <div className="space-y-1 mb-2">
                      {form.interests.map((v, i) => (
                        <div key={i} className="flex items-center gap-2 bg-white border border-gray-200 rounded px-2 py-1">
                          ...
                          <button type="button" onClick={() => removeInterestFromForm(i)} className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-gray-400 hover:text-red-500" aria-label="Remove interest">
                            <X className="h-4 w-4" />
                          </button>
                        </div>
                      ))}
                    ...
                    <div className="space-y-1 mb-2">
                      {form.goals.map((g, i) => (
                        <div key={i} className="flex items-center gap-2 bg-white border border-gray-200 rounded px-2 py-1">
                          ...
                          <button type="button" onClick={() => removeGoalFromForm(i)} className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-gray-400 hover:text-red-500" aria-label="Remove goal">
                            <X className="h-4 w-4" />
                          </button>
                        </div>
                      ))}
