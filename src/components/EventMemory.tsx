...
export function EventMemoryComponent({ eventId }: { eventId: string }) {
  ...
  const handleUpload = async (file: File) => {
    ...
    setUploading(true)
    setUploadError(null)
    const { error } = await uploadMemory(eventId, file)
    if (error) {
  ...
  const handleRecorded = (recorded: File) => {
    void handleUpload(recorded)
  }
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <label className={`inline-flex items-center min-h-[44px] px-4 py-2 bg-gray-100 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-200 ${uploading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
          <input
            type="file"
        ...
        <AudioRecorder onRecorded={handleRecorded} disabled={uploading} />
            ...
            <div key={m.id} className="relative group">
              ...
              {user && m.user_id === user.id && (
                <button
                  ...
                  className="absolute top-1 right-1 p-2 bg-black/60 text-white rounded-md hover:bg-black/80 transition-colors"
                >
