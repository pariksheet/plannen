...
import { ArrowLeft, Expand, Image as ImageIcon, Pencil, Share2, Trash2, X } from 'lucide-react'
    ...
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      <div className="mt-3 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {!editing ? (
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mt-1 break-words">{story.title}</h1>
          ) : (
            <input
              value={draftTitle}
              onChange={e => setDraftTitle(e.target.value)}
              className="w-full text-2xl sm:text-3xl font-bold text-gray-900 mt-1 border-b border-gray-300 focus:border-indigo-500 outline-none"
              aria-label="Title"
            />
          )}
        </div>
        {!editing && isOwner && (
          <div className="flex items-center gap-2 flex-wrap sm:flex-shrink-0">
            <button type="button" onClick={() => setShowShareModal(true)} className="min-h-[44px] text-sm text-gray-700 hover:bg-gray-100 px-3 py-2 rounded inline-flex items-center gap-1.5"><Share2 className="h-4 w-4" /> Share</button>
            <button type="button" onClick={() => setEditing(true)} className="min-h-[44px] text-sm text-gray-700 hover:bg-gray-100 px-3 py-2 rounded inline-flex items-center gap-1.5"><Pencil className="h-4 w-4" /> Edit</button>
            <button type="button" onClick={() => setPickerOpen(true)} className="min-h-[44px] text-sm text-gray-700 hover:bg-gray-100 px-3 py-2 rounded inline-flex items-center gap-1.5"><ImageIcon className="h-4 w-4" /> Cover</button>
            <button type="button" onClick={() => setConfirmDelete(true)} className="min-h-[44px] text-sm text-red-600 hover:bg-red-50 px-3 py-2 rounded inline-flex items-center gap-1.5"><Trash2 className="h-4 w-4" /> Delete</button>
          </div>
        )}
      </div>
      ...
      {showShareModal && (
        <StoryShareModal
          ...
          onSuccess={() => { setShowShareModal(false); refresh() }}
        />
      )}
