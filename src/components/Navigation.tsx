...
export function Navigation({ currentView, onViewChange, onSignOut, onInviteClick }: NavigationProps) {
  ...
  const primaryActive = currentView === 'groups' && primaryGroup != null && activeGroupId === primaryGroup.id
  ...
  // Tier 0 is single-user — the social feeds (Family/Friends/Groups) have no
  // backend wiring and silently render empty. Hide them; they re-appear in
  // Tier 1+ when multi-user SaaS surfaces are finished.
  const showSocialTabs = !isTierZero()
  const tabs: { id: View; label: string; icon: typeof LayoutDashboard }[] = [
    ...
    ...(showSocialTabs
      ? [
          ...
          { id: 'groups' as const, label: 'My Groups', icon: UsersRound },
        ]
      : []),
  ]
  const showInvite = !isTierZero()

  // The "My Groups" tab should light up only when on the groups view without
  // a primary-group filter — otherwise both tabs would highlight at once.
  const isTabActive = (id: View) =>
    id === 'groups' ? currentView === 'groups' && !primaryActive : currentView === id
            ...
            const tab = (
              <button
                ...
                className={`inline-flex items-center px-3 py-2 rounded-md text-sm font-medium ${
                  isTabActive(id) ? 'bg-indigo-100 text-indigo-700' : 'text-gray-600 hover:bg-gray-100'
                }`}
              >
                {label}
              </button>
            )
            if (id === 'groups' && primaryGroup) {
              return (
                <span key="primary-then-groups" className="contents">
                  <Link
                    key="primary-group"
                    to={`/dashboard?view=groups&group_id=${primaryGroup.id}`}
                    className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-md text-sm font-medium ${
                      primaryActive ? 'bg-indigo-100 text-indigo-700' : 'text-gray-600 hover:bg-gray-100'
                    }`}
                    title={`${primaryGroup.name} (primary group)`}
                  >
                    <Star className="h-3.5 w-3.5 fill-current" aria-hidden />
                    <span className="max-w-[140px] truncate">{primaryGroup.name}</span>
                  </Link>
                  {tab}
                </span>
              )
        ...
        <div className="flex items-center gap-2">
          ...
          {canInstall && (
            <button
              ...
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-gray-600 hover:bg-gray-100 text-xs font-medium"
              title="Install Plannen as an app"
            >
                ...
                const item = (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 text-left text-sm font-medium ${
                      isTabActive(id) ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'
                    }`}
                  >
                  ...
                  return (
                    <span key="primary-then-groups" className="contents">
                      <Link
                        ...
                        onClick={() => setMobileMenuOpen(false)}
                        className={`w-full flex items-center gap-3 px-4 py-3 text-left text-sm font-medium ${
                          primaryActive ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'
                        }`}
                      >
