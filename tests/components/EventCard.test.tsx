...
describe('EventCard compact', () => {
  it('renders the event title in compact mode', () => {
    ...
    expect(screen.getByText('Milo swimming')).toBeInTheDocument()
  })
  ...
  it('hides the kebab when it would render zero items', () => {
    ...
    // No showActions => no WhatsApp share mobile entry; no onEdit/onDelete =>
    // nothing left for the kebab to do.
  ...
  it('shows the mailbox icon for #mbsync events', () => {
    asOrganizer()
    render(<EventCard event={makeEvent({ hashtags: ['mbsync'] })} viewMode="compact" />)
  ...
  it('does not show the mailbox icon for non-mbsync events', () => {
    asOrganizer()
    render(<EventCard event={makeEvent({ hashtags: ['family'] })} viewMode="compact" />)
