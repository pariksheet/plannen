...
# then runs `npx plannen init --mode=local_sb --non-interactive` and asserts
...
# Make sure Tier 0 PG isn't running (so plannen init can start it cleanly).
...
# ── 4. Run plannen init. This is the thing under test. ───────────────────────
step "4. npx plannen init --mode=local_sb --non-interactive"
(cd "$ROOT" && npx plannen init --mode=local_sb --non-interactive)
