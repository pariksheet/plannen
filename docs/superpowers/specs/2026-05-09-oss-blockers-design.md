...
2. **Genericize my personal email in the plugin spec.** `user@example.com` appears once in `docs/superpowers/specs/2026-05-09-plannen-plugin-architecture-design.md` as an example. Replace with `you@example.com`.
3. **Audit `.mcp.json` (local-only file) before publishing.** Currently has `PLANNEN_USER_EMAIL=user@example.com`. Since it's not committed, no history concern — but the maintainer should regenerate it from `.mcp.json.example` after the plugin is installed (since the plugin replaces direct `.mcp.json` use anyway).
