...
describe('extractDomain', () => {
  it('extracts hostname from a full URL', () => {
    expect(extractDomain('https://www.zomerkampen.example/inschrijven')).toBe('zomerkampen.example')
  })
...
Restart the MCP server. In Claude Code, ask: `Create a test event called "Source Test" with enrollment_url "https://www.zomerkampen.example" and start_date "2027-06-01T10:00:00Z".`
...
Expected: one row with `domain = 'zomerkampen.example'` and `last_analysed_at = null`.
...
In Claude Code: `Create an event called "Linde Summer Camp" with enrollment_url "https://www.zomerkampen.example/zomerkamp" and start_date "2027-07-01T10:00:00Z".`

Expected:
1. Claude creates the event
2. Claude sees `source.last_analysed_at: null` in the response
3. Claude fetches `https://www.zomerkampen.example/zomerkamp` (or the homepage)
...
Expected: one row with `ref_type = 'enrollment_url'` and `domain = 'zomerkampen.example'`.
...
4. Response mentions Linde alongside any web results
