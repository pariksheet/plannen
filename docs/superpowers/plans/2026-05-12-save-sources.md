...
When the user singles out **one** link from a previously presented shortlist with positive sentiment — *"X looks good"*, *"let's go with X"*, *"send X to whatsapp"*, *"share X with Mara"*, *"let's look at X"*, *"check X out"* — end the reply with exactly one line:
