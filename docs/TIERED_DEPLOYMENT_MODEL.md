...
**Default tier.** Tier 0 is the typical default for `npx plannen init --mode=local_pg`. Tier 1 stays available via `--mode=local_sb`. The OSS-release framing is "runs with just Node" rather than "runs with Docker + Supabase."
...
It records row counts on Tier 1, runs `export-seed.sh`, stops the Tier 1 stack (Tier 0 reuses port 54322), flips `PLANNEN_TIER` in `.env`, runs `npx plannen init --mode=local_pg --non-interactive`, then re-counts on Tier 0 and reports a per-table diff. Pass `--yes` to wipe an existing `~/.plannen/pgdata` first (destructive).
...
npx plannen init --mode=local_pg
# init auto-restores the seed + photos on first run.
```
