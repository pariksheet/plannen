...
// Generic "what I did / measured" log (Phase 2 of /log). activity is free-form.
// log_activity also ticks a conservatively-matched active routine server-side.
async function logActivity(args: {
  ...
  if (!args.activity || !args.activity.trim()) throw new Error('activity is required')
  const activity = args.activity.trim()
  const norm = activity.toLowerCase()
  const userId = await uid()
  const tz = await getUserTimezone()
  const occurredAt = args.occurred_at ? parseInUserTz(args.occurred_at, tz) : new Date()
  const occurredIso = occurredAt.toISOString()
  const completedOn = args.occurred_at && /^\d{4}-\d{2}-\d{2}$/.test(args.occurred_at) ? args.occurred_at : occurredIso.slice(0, 10)
  const normOf = (v: unknown) => String(v ?? '').trim().toLowerCase()
  return await withUserContext(userId, async (c) => {
    const { rows } = await c.query(
...
async function listActivityLogs(args: { activity?: string; from?: string; to?: string; family_member_id?: string | null; limit?: number }) {
  const userId = await uid()
  return await withUserContext(userId, async (c) => {
    const params: unknown[] = [userId]
    const where: string[] = ['user_id = $1']
    if (args.activity) {
      params.push(args.activity.trim().toLowerCase())
    ...
    if (args.family_member_id !== undefined) {
      if (args.family_member_id === null) where.push('family_member_id is null')
      else {
...
async function deleteActivityLog(args: { id: string }) {
  const userId = await uid()
  return await withUserContext(userId, async (c) => {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'log_activity':       result = await logActivity(args as Parameters<typeof logActivity>[0]); break
      case 'list_activity_logs': result = await listActivityLogs(args as Parameters<typeof listActivityLogs>[0]); break
      case 'delete_activity_log': result = await deleteActivityLog(args as Parameters<typeof deleteActivityLog>[0]); break
