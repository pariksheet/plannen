...
import { generateSessionDates, parseInUserTz } from './recurrence.js'

describe('parseInUserTz — timezone-naive input is interpreted in the user tz', () => {
  // 11M-call regression: create_event received "2026-06-06T09:30:00" (no offset)
  // for a 09:30 Europe/Brussels call. The naive string was resolved against the
  // server timezone (UTC on Supabase edge) → stored 09:30Z → displayed 11:30 local.
  // Assertions deliberately use timezones ≠ the dev host tz so the suite catches
  // host-tz-dependent parsing on any machine.
  it('interprets a naive datetime in the given tz, not the host/server tz', () => {
    expect(parseInUserTz('2026-06-06T09:30:00', 'Europe/Brussels').toISOString())
      .toBe('2026-06-06T07:30:00.000Z') // CEST, +02:00
    expect(parseInUserTz('2026-12-05T09:30:00', 'Europe/Brussels').toISOString())
      .toBe('2026-12-05T08:30:00.000Z') // CET, +01:00
    expect(parseInUserTz('2026-06-06T09:30:00', 'Asia/Kolkata').toISOString())
      .toBe('2026-06-06T04:00:00.000Z') // +05:30, no DST
    expect(parseInUserTz('2026-06-06T09:30:00', 'UTC').toISOString())
      .toBe('2026-06-06T09:30:00.000Z')
  })

  it('accepts naive datetimes without seconds', () => {
    expect(parseInUserTz('2026-06-06T09:30', 'Europe/Brussels').toISOString())
      .toBe('2026-06-06T07:30:00.000Z')
  })

  it('respects an explicit offset regardless of the tz argument', () => {
    expect(parseInUserTz('2026-06-06T09:30:00+02:00', 'Asia/Kolkata').toISOString())
      .toBe('2026-06-06T07:30:00.000Z')
  })

  it('respects a trailing Z regardless of the tz argument', () => {
    expect(parseInUserTz('2026-06-06T09:30:00Z', 'Europe/Brussels').toISOString())
      .toBe('2026-06-06T09:30:00.000Z')
  })

  it('interprets a date-only string as midnight in the given tz', () => {
    expect(parseInUserTz('2026-06-06', 'Europe/Brussels').toISOString())
      .toBe('2026-06-05T22:00:00.000Z')
  })
})

describe('generateSessionDates — timezone-naive parent start', () => {
  it('anchors sessions to the naive wall-clock time in `tz`, not the host tz', () => {
    // Pre-fix: `new Date('2026-06-06T09:30:00')` parsed in the host tz, so the
    // session anchor depended on where the server ran (UTC on edge → 11:30 local).
    const sessions = generateSessionDates(
      '2026-06-06T09:30:00',
      { frequency: 'weekly', days: ['SA'], until: '2026-06-13', session_duration_minutes: 60 },
      'Asia/Kolkata',
    )
    ...
    expect(sessions[0].end?.toISOString()).toBe('2026-06-06T05:00:00.000Z')
  })
})
...
describe('generateSessionDates — weekly with date-only `until`', () => {
  it('includes a session on the `until` day even when its UTC time is past midnight', () => {
    // Milo's Swimming Class regression: 12:30 Europe/Brussels (10:30 UTC) on Sat Jun 20.
...
describe('generateSessionDates — DST handling', () => {
  it('preserves 18:15 Europe/Brussels across the CEST→CET transition', () => {
    // Milo's inline-skating regression: parent created in CEST (UTC+2),
