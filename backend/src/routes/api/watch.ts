...
watch.patch('/:id', async (c) => {
  ...
  return await withUserContext(userId, async (db) => {
    ...
    // Push notification when a watcher reports a meaningful change. Tagged by
    // event_id so repeated patches collapse to one notification per event.
    if (parsed.data.has_unread_update === true && parsed.data.update_summary) {
      const { rows: ev } = await db.query(
        'SELECT title FROM plannen.events WHERE id = $1',
        [rows[0].event_id],
      )
      const title = ev[0]?.title ?? 'Watched event'
      void sendPush(userId, {
        title: `Update: ${title}`,
        body: parsed.data.update_summary,
        url: `/dashboard`,
        tag: `watch-${rows[0].event_id}`,
